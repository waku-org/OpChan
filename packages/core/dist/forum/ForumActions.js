...
export class ForumActions {
     ...
     */
    validatePermission(action, currentUser, _isAuthenticated) {
        const verificationStatus = currentUser?.verificationStatus || EVerificationStatus.WALLET_UNCONNECTED;
        switch (action) {
            case 'createCell':
    ...
    -------------------------------------------------------------------*/
    async createPost(params, updateStateFromCache) {
        const { cellId, title, content, currentUser, isAuthenticated } = params;
        const validation = this.validatePermission('createPost', currentUser, isAuthenticated);
        if (!validation.valid) {
            return {
                success: false,
        ...
        try {
            ...
            if (!signed) {
                const status = await this.delegationManager.getStatus(currentUser.address, currentUser.walletType);
                return {
                    success: false,
            ...
            const transformedPost = await transformPost(signed);
            if (!transformedPost) {
    ...
    async createComment(params, updateStateFromCache) {
        const { postId, content, currentUser, isAuthenticated } = params;
        // Use unified validation
        const validation = this.validatePermission('createComment', currentUser, isAuthenticated);
        if (!validation.valid) {
            return {
                success: false,
                error: validation.error,
        ...
        try {
            ...
            messageManager
                ...
                .catch((err) => console.error('Background send failed:', err))
                .finally(() => localDatabase.setSyncing(false));
            const transformed = await transformComment(signed);
            if (!transformed) {
    ...
    async createCell(params, updateStateFromCache) {
        const { name, description, icon, currentUser, isAuthenticated } = params;
        // Use unified validation
        const validation = this.validatePermission('createCell', currentUser, isAuthenticated);
        if (!validation.valid) {
            return {
                success: false,
                error: validation.error,
        ...
        try {
            ...
            messageManager
                ...
                .catch((err) => console.error('Background send failed:', err))
                .finally(() => localDatabase.setSyncing(false));
            const transformedCell = await transformCell(signed);
            if (!transformedCell) {
    ...
    /* ------------------------------------------------------------------
       VOTING
    -------------------------------------------------------------------*/
    async vote(params, updateStateFromCache) {
        const { targetId, isUpvote, currentUser, isAuthenticated } = params;
        // Use unified validation
        const validation = this.validatePermission('vote', currentUser, isAuthenticated);
        if (!validation.valid) {
            return {
                success: false,
                error: validation.error,
    ...
    /* ------------------------------------------------------------------
       MODERATION
    -------------------------------------------------------------------*/
    async moderatePost(params, updateStateFromCache) {
        const { cellId, postId, reason, currentUser, isAuthenticated, cellOwner } = params;
        if (!isAuthenticated || !currentUser) {
            return {
                success: false,
                error: 'Authentication required. You need to verify Ordinal ownership to moderate posts.',
            };
        }
    ...
    async moderateComment(params, updateStateFromCache) {
        const { cellId, commentId, reason, currentUser, isAuthenticated, cellOwner, } = params;
        if (!isAuthenticated || !currentUser) {
            return {
                success: false,
                error: 'Authentication required. You need to verify Ordinal ownership to moderate comments.',
            };
        }
        if (currentUser.address !== cellOwner) {
            return {
    ...
    async moderateUser(params, updateStateFromCache) {
        const { cellId, userAddress, reason, currentUser, isAuthenticated, cellOwner, } = params;
        if (!isAuthenticated || !currentUser) {
            return {
                success: false,
                error: 'Authentication required. You need to verify Ordinal ownership to moderate users.',
            };
        }
        if (currentUser.address !== cellOwner) {
            return {
